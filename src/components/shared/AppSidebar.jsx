"use client";
import { HiArrowLeft, HiBookOpen, HiChartPie, HiCog, HiDotsHorizontal, HiDotsVertical, HiUser, HiUserCircle, HiUserGroup, HiUsers, HiViewList } from "react-icons/hi";
    ...
    <div className="sticky top-0 h-screen">
      <div className="flex justify-between bg-white">
        ...
        <button onClick={toggleOpen} className="p-5 rounded-lg transition-all duration-300 hover:bg-gray-300">
          {isOpen ? <HiDotsHorizontal /> : <HiDotsVertical />}
        </button>
      </div>
      <div className={`transition-all duration-300 ${isOpen ? 'w-64' : 'w-0'} h-full`}>
        <Drawer open={isOpen} onClose={toggleClose} className={`!px-0 !py-0 transition-all duration-300 ${isOpen ? 'w-64' : 'w-0'} h-full`}>
